...
UX contract implemented here (matched to the Flask API in api.py):
  ...
  * Whenever the LOCAL PLAYER makes a move (any mode, either colour),
    their move is confirmed with a WHITE flash on the from/to squares.
    This white flash has priority over pending opponent prompts.

  * Whenever the OPPONENT moves (Stockfish engine, Lichess opponent,
    or a move that arrived on the shared HVH board from elsewhere), the
    board blinks RED on the source square and GREEN on the destination
    square (orange if it gives check) and waits for the player to
    physically execute the move. Identical for White and Black.

  * Stockfish mode supports the human playing either colour
    (config: play_as = "white" | "black" | "alternate"). The engine is
    asked to move via POST /sf-play whenever it is the engine's turn.

  * HVH mode: the server holds one shared board (/hvh-*). Both local
    players simply move pieces; every accepted move flashes white. The
    loop also polls GET /hvh-status so moves posted to the server by
    anything else (e.g. a second physical board) are mirrored red->green.

  * Lichess mode: the loop POSTs /update-lichess-game so the server
    actually starts streaming game state (without that call
    /lichess-status stays frozen at "found"), handles gameFull
    ("initialFen"), infers our colour if the server does not provide
    it, and mirrors every remote move red->green.

Server caveats this client works around:
  - The server's lichess results dict drops the player's colour when
    the game state changes to "playing"; the client infers colour from
    the physical board (see _apply_lichess_moves). A one-line server
    patch (include "color": self.color in update()) makes it instant.
  ...
  - HVH's HumanGame.make_move() does not validate legality server-side;
    this client only sends legal moves.
  - "auto" mode can only detect stockfish or lichess (the server gives
    no signal that an HVH game is active). Force mode="hvh" for HVH.
"""
...
class MirrorTask:
    ...
    match_keys: Set[str] = field(default_factory=set)     # "<from><to>" gestures
...
class GameLoop:
    def __init__(self, cfg: Config, matrix, leds: LedView, client: GameClient,
                 log: logging.Logger):
        self.cfg = cfg
        self.matrix = matrix
        self.leds = leds
    ...
    def _tick(self):
        ...
        # Auto-switch to lichess whenever a live lichess game is present.
        # This is deliberately not gated on state so switching also happens
        # from stockfish game_over/pending_execution after a lichess game
        # has been started.
    ...
    def _kick_engine_if_needed(self):
        """
    ...
    def _poll_hvh(self):
        """
        Ingest moves that reached the shared server board from elsewhere
        (e.g. a second physical board) by diffing the server FEN against
        our logical board, then prompt them red -> green.
        """
        if self.state == "game_over":
            return
