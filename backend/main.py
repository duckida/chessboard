...
UX contract implemented here (matched to the Flask API in api.py):
  ...
  * Whenever the LOCAL PLAYER makes a move (any mode, either colour),
    ...
    (plus the rook squares when they castle).

  * Whenever the OPPONENT moves (Stockfish engine, Lichess opponent, or
    a move that arrived on the shared HVH board from elsewhere), the
...
class GameLoop:
    ...
    def _tick(self):
        ...
        if (self.mode != "lichess" and self.cfg.mode == "auto"
                and self.state == "awaiting_human" and not self.mirror_tasks
                and not self._engine_inflight
    ...
    def _apply_lichess_moves(self, moves: List[str]):
        ...
        for mv in moves[self.lichess_ply_count:]:
            ...
            if self.human_color is None:
                ...
                #   pieces already moved to the destination  -> that was us
                #   pieces still on their origin squares     -> remote move
            ...
            if self.human_color is not None and mover != self.human_color:
                # Remote opponent move: prompt red -> green (mirrors the
                # push internally; auto-completes if already executed).
        ...
        if moves:
            ...
            if before is not None:
                ...
                if (self.human_color is not None and mover != self.human_color
                    ...
                    # prompt it red -> green.
                    self.board = before
                    self._mirror_move(moves[-1])   # pushes back to new_board
